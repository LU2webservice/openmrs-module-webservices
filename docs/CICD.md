...
| False positives | Beleid plus een bestand om meldingen uit te zetten | .grype.yaml en dit bestand |
...
3. Vindt hij iets ernstigs (high of critical), dan faalt deze check.
...
Waar we dat doen:
- Grype: een regel in .grype.yaml met het CVE-nummer en een reden.
- CodeQL: het alert in de Security-tab afwijzen met een reden.

We zetten dus nooit een hele check uit, alleen losse meldingen met uitleg. Zo kun je
later terugvinden waarom iets is genegeerd.
...
SCA-scan (Grype) staat er bewust niet bij als verplicht. Die kan namelijk falen op
een lek in een OpenMRS-dependency die we zelf niet kunnen updaten. Het lek is dan
nog wel te zien in de Security-tab en zetten we uit in .grype.yaml.
